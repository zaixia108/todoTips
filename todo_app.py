...
from PySide6.QtWidgets import (QSystemTrayIcon, QMenu, QMessageBox, QStyle)  # 添加 QStyle
...
class HotkeyWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            while self.running:
                keyboard.wait("Esc")  # 等待0.5秒，允许检查running标志
...
class TodoApplication:
    ...
    def __init__(self):
        ...
        self.last_clipboard_text = ""  # 添加：跟踪上一个剪贴板内容
    ...
    def create_tray_icon(self):
        ...
        icon = self.main_window.style().standardIcon(
            QStyle.SP_MessageBoxInformation  # 修正：使用 QStyle.SP_MessageBoxInformation
        )
    ...
    def setup_hotkeys(self):
        """设置全局快捷键"""
        """设置全局快捷键"""
        quick_add_key = str(self.settings.value('hotkey/quick_add', 'ctrl+shift+a'))
        show_window_key = str(self.settings.value('hotkey/show_window', 'ctrl+shift+t'))
    ...
    def quick_add_from_clipboard(self):
        ...
        try:
            ...
            if clipboard_text and clipboard_text.strip() and clipboard_text != self.last_clipboard_text:  # 修改：只在内容不同时添加
                self.last_clipboard_text = clipboard_text  # 更新跟踪变量
                # 添加到当日待办
            ...
            elif clipboard_text == self.last_clipboard_text:
                self.notification_manager.show_notification(
                    "提示",
                    "剪贴板内容未变化"
                )
